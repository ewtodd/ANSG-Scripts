...
def build_table(xml_file):
    ...
    for board_idx, board in enumerate(root.findall('board')):
        ...
        data = {row_name: {channel: None for channel in channels} 
                for row_name in parameter_keys.values()}
        
        # Identify enabled channels for this board
        enabled_channels = set()
        _, enabled_values = extract_parameter_values(xml_file, 'SRV_PARAM_CH_ENABLED', board)
        ...
        for parameter_key, row_name in parameter_keys.items():
            globalvalues, chvalues = extract_parameter_values(xml_file, parameter_key, board)
        ...
        for row_name in data.keys():
            data[row_name] = {k:v for k,v in data[row_name].items() 
                             if k in enabled_channels}
